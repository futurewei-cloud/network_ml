def ec_internal_print(template, config, print_args = None):
    '''uses print and termcolor to get a colored output in the terminal使用print和termcolor在终端中获得彩色输出
    Input :
        template : A string with the format of the output输出格式的字符串
    ...
    #Get a full config object (optional properties)获取完整配置对象（可选属性）
    full_config = []
    for c in config:
        obj = {}

        #text is a required argument文本是必需参数
        try : obj['text'] = c['text']
        except KeyError: raise ValueError("could't find property text in {}".format(c))

        #color and bg_color are optional arguments  颜色和bg_color颜色是可选参数
        try :
    ...
    #Get the colored version of the input texts  获取输入文本的彩色版本
